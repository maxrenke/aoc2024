        ...
        if c in string.digits:
            ...
            for cell in find_adjacent_cells(schematic, i, j):
                if is_symbol(schematic, cell[0], cell[1]):
                    symbol = schematic[cell[0]][cell[1]]
                    key = (cell[0], cell[1])
                    adj = True
            ...
            if len(part) > 0:
                ...
                for cell in find_adjacent_cells(schematic, i, j-1):
                    if is_symbol(schematic, cell[0], cell[1]):
                        ...
                        adj = True
                if adj:
                    print(part + " adjacent to symbol: " + symbol)
                    if symbol == '*':
                        if key in gears:
                            gears[key].append(int(part))
                        else:
                            gears[key] = [int(part)]
...
for gear in gears:
    ...
    if len(parts) == 2:
        ...
        for part in parts:
            ratio *= part
        print(str(gear) + " ratio: " + str(ratio))
        sum += ratio

print("Sum of all gears: " + str(sum))